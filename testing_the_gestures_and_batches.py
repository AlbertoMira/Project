# -*- coding: utf-8 -*-
"""Testing the gestures and batches.ipynb
...
# Training and testing in batches

# Taking the signals and split them into small batches (the size of the batch is defined by the variable "constant")
l=[]
l2=[]
a=np.zeros((1,4))
constant=201
for j in range(1,101):
  for e in c[j-1]: #Strong grasp
      if a.shape[0]<constant:
        if int(e[0])!=0 and int(e[1])!=0 and int(e[2])!=0 and int(e[3])!=0:
          a=np.append(a, [[int(e[0]),int(e[1]),int(e[2]),int(e[3])]], axis=0)
      else:
        a=a[1:]
        l.append(a)
        l2.append(0)
...
a=np.zeros((1,4))
for j in range(500,593):
  for e in c[j-1]: #No movement
      if a.shape[0]<constant:
        if int(e[0])!=0 and int(e[1])!=0 and int(e[2])!=0 and int(e[3])!=0:
          a=np.append(a, [[int(e[0]),int(e[1]),int(e[2]),int(e[3])]], axis=0)
      else:
...
train_in_batches = np.array(l)
labels_in_batches = np.array(l2)

X_train, X_test, y_train, y_test = train_test_split(train_in_batches, labels_in_batches, test_size=0.2) # Data split
...
# Model
model = keras.Sequential([
  keras.layers.Flatten(input_shape=(200,4)),
  keras.layers.Dense(120, activation='relu'),
  keras.layers.Dense(120, activation='relu'),
  keras.layers.Dense(6, activation='softmax') 
])
